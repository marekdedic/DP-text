...
For each input, the center of the cluster in which its representation falls is denoted as
\[ \mathvec{\mu} \left( \mathvec{r} \right) = \argmin_{\mathvec{\mu_k^c}} \left\lVert \mathvec{r} - \mathvec{\mu}_k^c \right\rVert \]
and the distance of its representation from the center of the cluster is denoted as
...
The cluster index is refreshed periodically between optimization passes. The frequency of the updates is a hyper-parameter of the method, although \cite{rippel_metric_2015} suggest updating it before each optimization pass.
